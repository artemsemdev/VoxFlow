...
struct PreflightBuilderTests {
    func builder(app: FrontmostApp = FrontmostApp(name: "Mail", bundleID: "com.apple.mail"), secure: Bool = false,
        ...
        -> (PreflightBuilder, FakePermissions, Counter, CapturedApps, CapturedApps) {
        ...
        let frontmostCaptured = CapturedApps()
        let builder = PreflightBuilder(frontmost: FakeFrontmost(app: app, secure: secure), permissions: permissions,
                                       ...
                                       onFrontmostCaptured: { app in frontmostCaptured.withLock { $0.append(app) } })
        return (builder, permissions, captured, capturedApps, frontmostCaptured)
    }

    @Test("happy path: everything granted, model loaded, focus captured once with the frontmost app preflight validated")
    func happy() async {
        let (b, _, captured, capturedApps, frontmostCaptured) = await builder()
        ...
        // `onFrontmostCaptured` fires on the clean path with the same app `captureFocus` saw.
        #expect(frontmostCaptured.withLock { $0 } == [FrontmostApp(name: "Mail", bundleID: "com.apple.mail")])
    }

    @Test("excluded app and secure input gate before permissions; focus is not captured")
    func gates() async {
        let (a, _, capA, _, frontmostA) = await builder(app: FrontmostApp(name: "1Password", bundleID: "com.1password.1password"))
        #expect(await a.preflight().excludedApp == "1Password")
        #expect(capA.withLock { $0 } == 0)
        #expect(frontmostA.withLock { $0 } == [])
        let (s, _, _, _, frontmostS) = await builder(secure: true)
        #expect(await s.preflight().secureInput)
        #expect(frontmostS.withLock { $0 } == [])
    }

    @Test("not-determined microphone permission is requested once; denied stays denied")
    func microphone() async {
        let (b, perms, cap, _, _) = await builder(mic: .notDetermined, requestResult: .granted)
        #expect(await b.preflight().microphone == .granted)
        #expect(perms.requests == 1)
        #expect(cap.withLock { $0 } == 1)
        let (d, _, capD, _, _) = await builder(mic: .denied)
