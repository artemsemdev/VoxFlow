...
struct FlowBarRenderTests {
    ...
    private static let cases: [RenderCase] = [
        ...
        RenderCase(name: "copied", state: .copied(.noTextField), elapsed: 0, mode: .pushToTalk),
        RenderCase(name: "accessibility-denied", state: .copied(.accessibilityDenied), elapsed: 0, mode: .pushToTalk),
        RenderCase(name: "insertion-failed", state: .copied(.insertionFailed), elapsed: 0, mode: .pushToTalk),
