...
| **Symlink escape** — a symlink *inside* an allowed root (`~/Audio/secrets -> /etc`) used to reference `~/Audio/secrets/passwd`. | `ResolveRealPath` canonicalizes the candidate path — resolving symlinks on the longest existing prefix, including intermediate directory links — before the root check. Allowed roots are canonicalized the same way, so a symlinked root still matches and a link escaping the root does not. A broken symlink resolves to its declared target, so it cannot reference outside the root either. | [`PathPolicy.cs`](../../src/VoxFlow.McpServer/Security/PathPolicy.cs) |
...
| **TOCTOU between policy check and file IO.** Static symlink escape is now resolved (see the defenses table — `ResolveRealPath` canonicalizes symlinks at validation time). What remains is the *time-of-check/time-of-use* window: `ValidateInputPath` resolves and checks the path, then the actual `File.Open*` / ffmpeg launch happens a moment later. An attacker who can swap a component for a symlink *in that window* can still defeat the check. | The window is small (microseconds) and requires write access to a directory inside an allowed root at exactly the wrong instant. | Restrict ownership/permissions of the allowed roots to the user running the MCP server so no other principal can swap entries. Fully closing this needs open-by-handle + `O_NOFOLLOW`/`fstat` or OS sandboxing, which is out of scope for the path-string policy. |
...
2. **Symlinks inside allowed roots** — **resolved** (#85). `ResolveRealPath` canonicalizes symlinks (including intermediate directory links and symlinked roots) before the allowed-root check, so a link escaping the root is denied. The residual risk is the TOCTOU window below, not static symlink traversal.
3. **TOCTOU races** — micro-window between path validation and file IO; an attacker who swaps a path component for a symlink in that instant can still defeat the check. Restrict ownership/permissions of the allowed roots to the server's user account. Fully closing it requires open-by-handle + `O_NOFOLLOW` or OS sandboxing, out of scope for the path-string policy.
...
- Test coverage: [`tests/VoxFlow.McpServer.Tests/PathPolicyTests.cs`](../../tests/VoxFlow.McpServer.Tests/PathPolicyTests.cs) and [`PathPolicySymlinkTests.cs`](../../tests/VoxFlow.McpServer.Tests/PathPolicySymlinkTests.cs) (symlink escape, symlinked root, broken symlink).
